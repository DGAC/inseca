...
#  - ensure the storage driver is overlay2 (is it devicemapper in the live Linux setup)
mkdir -p "$LIVE_DIR/etc/default"
cat >> "$LIVE_DIR/etc/default/docker" << EOF
DOCKER_OPTS="--storage-driver=overlay2 -g /data/docker"
EOF
